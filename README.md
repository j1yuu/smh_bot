...
- `main.py` — самый удобный корневой entrypoint без необходимости вручную выставлять `PYTHONPATH`.

## Структура проекта

```text
main.py                # рекомендуемая точка входа
src/
...
## Как удобнее всего запускать приложение
...
Если запустить `main.py` без аргументов, приложение сразу откроет интерактивный чат.
